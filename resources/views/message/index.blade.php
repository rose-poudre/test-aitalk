              ...
              <tr class="hover:bg-grey-lighter">
                ...
                <!-- 🔽 詳細画面へのリンク -->
                    <a href="{{ route('message.show',$message->id) }}">
                        ...
                        <h3 class="text-left font-bold text-lg text-grey-dark">{{$message->reply}}</h3>
                    </a>
                    <div class="flex">
                        <!-- favorite 状態で条件分岐 -->
                        @if($message->users()->where('user_id', Auth::id())->exists())
                        <!-- unfavorite ボタン -->
                        <form action="{{ route('unfavorites',$message) }}" method="POST" class="text-left">
                          @csrf
                        ...
                        @else
                        <!-- favorite ボタン -->
                        <form action="{{ route('favorites',$message) }}" method="POST" class="text-left">
                          @csrf
